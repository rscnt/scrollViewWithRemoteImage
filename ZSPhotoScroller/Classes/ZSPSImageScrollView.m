...
//        _image = nil;
        ...
        _image = [[UIImageView alloc] initWithFrame:(CGRect){ CGPointZero, image.size }];
...
- (instancetype)initWithURLOfImage:(NSURL *)url {
  self = [super init];
  if (self) {
     //[self setURLOfImageToScroll:url];
  }
  return self;
}
...
- (BOOL)setUIImageToScroll:(UIImage *)image {
    ...
    _tillingView.frame = _image.bounds;
    [_image addSubview:_tillingView];
    [self configureForImageSize:_image.image.size];
  return YES;
}
...
- (void)setMaxMinZoomScalesForCurrentBounds {
  ...
  // fill width if the image and phone are both portrait or both landscape;
  // otherwise take smaller scale
  BOOL imagePortrait = _imageSize.height > _imageSize.width;
  BOOL phonePortrait = boundsSize.height > boundsSize.width;
  CGFloat minScale =
      imagePortrait == phonePortrait ? xScale : MIN(xScale, yScale);

  // on high resolution screens we have double the pixel density, so we will be
  // seeing every pixel if we limit the
...
- (void)layoutSubviews {
  ...
  CGSize boundsSize = self.bounds.size;
  CGRect frameToCenter = _image.frame;
...
- (void)recoverFromResizing {
  ...
  // Step 1: restore zoom scale, first making sure it is within the allowable
  // range.
  CGFloat maxZoomScale = MAX(self.minimumZoomScale, _scaleToRestoreAfterResize);
  self.zoomScale = MIN(self.maximumZoomScale, maxZoomScale);

  // Step 2: restore center point, first making sure it is within the allowable
  // range.

  // 2a: convert our desired center point back to our own coordinate space
  CGPoint boundsCenter =
      [self convertPoint:_pointToCenterAfterResize fromView:_image];
